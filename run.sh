...
process_ipsw() {
  ...
  # Iterate over each Identifier in SupportedProductTypes array
  ...
  python3 -m pyimg4 im4p extract -i "$KernelCacheName" -o "kernelcache".raw
  rm -f "$KernelCacheName"
  while true; do
    Identifier=$(/usr/libexec/PlistBuddy -c "print SupportedProductTypes:${Index}" BuildManifest.plist 2>/dev/null)
    if [ -z "$Identifier" ]; then
      break  # No more items in the array
    fi
    
    # Offsets extracting
    if [[ "$Identifier" =~ "iPhone".* || "$Identifier" =~ "iPad".* ]]; then
        offsetexporter  -i "kernelcache".raw \
  ...
  rm -f *.raw > /dev/null
  rm -f template_dynamic_info.h > /dev/null
}
# Argument ?
if [ $# -eq 1 ]; then
  input="$1"
